...
class PDF(FPDF):
    ...
    def chapter_body_filepath(
        self, filepath, collapse_source_newlines=True, verbose=False, std_font=14, word2vec_model = "glove-wiki-gigaword-100", use_punkt = True, keyword_threshold:int=1500
    ):
        """
        ...
        if self.split_paragraphs:
            ...
            self.multi_cell(w=0, h=th, txt=decoded_line, border=0) # "keep all text as one blob"
            self.ln()
    ...
    def chapter_body_fromURL(self, aURL:str, word2vec_model = "glove-wiki-gigaword-100", use_punkt = True, keyword_threshold:int=1500):
        """
